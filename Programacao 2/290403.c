...
struct PorcentagemVotosEtaria resultadoFaixaEtaria(struct DadosPesquisa *dadosPesquisa, int entradas) {
    struct PorcentagemVotosEtaria resultadoEtaria;
    int i = 0;
    for(i = 0; i<entradas; i++) {
        
        if(dadosPesquisa->aprovacaoGov == 1) 
        if(dadosPesquisa->idade < 25) {
            resultadoEtaria.menorQ25 = 100*(float)resultadoEtaria.menorQ25/entradas;
        } else if(dadosPesquisa->idade >=25 && dadosPesquisa->idade <55) {
            resultadoEtaria.entre25e55 = 100*(float)resultadoEtaria.entre25e55/entradas;
        } else {
            resultadoEtaria.acima55 = 100*(float)resultadoEtaria.acima55/entradas;
        }
